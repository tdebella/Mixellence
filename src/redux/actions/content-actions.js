...
export const firebaseContent = () => {
    return async (dispatch) => {
        //call startingload when going into function
        //await getDocs()
        //.then { on success dispatch stoppingLoad when everything fetched}
        //.catch { error something dispatch stoppingLoad }
        let aboutContent = [];

    await getDocs(aboutRef).then((res) => {
      res.docs.forEach((docs) => {
        aboutContent.push({ ...docs.data() });
        dispatch(contentActions.updateContent({ property: 'aboutUs' , data: aboutContent[0].summary}))
      })
    });
    ...
    getDocs(barRef).then((res) => {
      ...
      dispatch(contentActions.updateContent({ property: 'bartenders' , data: bartenderContent}))
    });
    ...
    getDocs(drinksRef).then((res) => {
      ...
      dispatch(contentActions.updateContent({ property: 'drinks' , data: drinksContent}))
    });
    ...
    getDocs(testRef).then((res) => {
      ...
      dispatch(contentActions.updateContent({ property: 'testimonials' , data: testimonialsContent}))
    });
    }
}
